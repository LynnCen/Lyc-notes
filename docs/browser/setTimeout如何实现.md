...
设置一个定时器，JavaScript 引擎会返回一个定时器的 ID。那通常情况下，当一个定时器的任务还没有被执行的时候，也是可以取消的，具体方法是调用 clearTimeout 函数，并传入需要取消的定时器的 ID。如下面代码所示：`clearTimeout(timer_id)`
...
![alt text](./img/长任务导致定时器被延后执行.png)
...
![alt text](./img/循环嵌套调用setTimeout.png)
