...
![alt text](./img/使用对称加密实现HTTPS.png)
...
基于以上两点原因，我们最终选择了一个更加完美的方案，那就是在**传输数据阶段依然使用对称加密，但是对称加密的密钥我们采用非对称加密来传输**。下图就是改造后的版本：

![alt text](./img/混合加密实现HTTPS.png)
