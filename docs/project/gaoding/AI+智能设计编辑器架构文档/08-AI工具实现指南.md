# 08 - AI工具实现指南

> **导航**：[📚 返回总目录](./README.md) | [⬅️ 上一篇：Agent工作流程](./07-Agent工作流程.md) | [➡️ 下一篇：性能优化与错误处理](./09-性能优化与错误处理.md)
>
> **所属**：AI+ 智能设计编辑器架构文档
>
> **核心价值**：学习各种AI工具的前端实现流程和关键技术

---

## 一、AI工具全景

系统提供了**10+种AI工具**，按功能分为三大类：

### 1.1 工具分类

| 类别 | 工具 | 功能 | 典型场景 |
|------|------|------|---------|
| **生成类** | AI抠图 | 自动分离前景与背景 | 去除产品图背景 |
|  | AI滤镜 | 图片风格转换 | 将照片转成油画风格 |
|  | 转矢量 | 位图转矢量 | Logo转可缩放的矢量图 |
| **编辑类** | AI改图 | 局部替换内容 | 把红色汽车改成蓝色 |
|  | AI消除 | 智能去除物体 | 移除照片中的路人 |
|  | AI扩图 | 智能扩展边界 | 将16:9的图扩展成1:1 |
|  | AI变清晰 | 图片超分辨率 | 低清图片变高清 |
| **分析类** | 图文分层 | 自动识别文字和图形 | 将海报拆分成可编辑的模板 |
|  | 溶图 | 图片融合 | 将两张图自然融合 |
|  | AI换脸 | 人脸替换 | 替换照片中的人脸 |
|  | AI阴影 | 自动生成阴影 | 为抠图元素添加真实阴影 |

### 1.2 工具注册

```typescript
// domains/editor/extensions/ai/src/hooks/use-image-actions.ts

export function useImageActions() {
  const actions: IAction[] = [
    {
      name: 'openInMatting',          // 抠图
      title: 'AI 抠图',
      filter: () => true,             // 总是可用
      valid,                          // 元素验证函数
    },
    {
      name: 'openInPaintReplace',     // 改图
      title: 'AI 改图',
      filter: createFilter('inPaintReplaceV2'),  // 权限控制
      valid,
    },
    {
      name: 'openInPaintRemove',      // 消除
      title: 'AI 消除',
      filter: () => !!(createFilter('inPaintRemove') || createFilter('inPaintRemoveV2')),
      valid,
    },
    // ... 其他工具
  ];

  return { actions, handleClickAction };
}
```

**关键设计**：

- `filter`：权限控制（是否启用该工具）
- `valid`：元素验证（当前元素是否可使用该工具）
- `cover`：工具的预览封面（静态图和视频）

---

## 二、AI抠图：自动分离前景与背景

### 2.1 功能描述

**输入**：带背景的图片  
**输出**：透明背景的前景图（PNG）

**典型场景**：
- 产品拍摄图去除背景
- 人物照片提取主体
- Logo素材处理

### 2.2 实现流程

```
用户操作流程：
1. 选中图片元素
2. 点击右侧面板"AI抠图"按钮
3. 系统自动发起抠图请求
4. 显示加载状态
5. 抠图完成，替换为透明背景图
6. 用户可以调整参数重新抠图
```

### 2.3 关键技术

**核心API调用**：

```typescript
// 调用抠图接口
const result = await fetch('/api/ai/matting', {
  method: 'POST',
  body: JSON.stringify({
    image_url: element.imageUrl,  // 原图URL
    algorithm: 'u2net',            // 或 'modnet'（人像专用）
  }),
});

// 返回结果
{
  "matted_url": "https://cdn.com/matted-image.png",  // 抠图后的URL
  "mask": "https://cdn.com/mask.png",                // 蒙版（可选）
}
```

**前端处理**：

```typescript
// 1. 触发抠图
postMessage({ type: 'openInMatting' });

// 2. 监听抠图结果
events.on('mattingCompleted', (result) => {
  // 更新元素图片URL
  editor.updateElement(element, {
    imageUrl: result.matted_url,
    mask: result.mask,  // 保存蒙版用于后续编辑
  });

  // 保存操作记录
  aiRecordsService.addRecord({
    tool_type: 'matting',
    origin_url: element.imageUrl,
    result_url: result.matted_url,
    element_id: element.id,
  });
});
```

**算法选择**：

- `u2net`：通用算法，适合各种物体
- `modnet`：人像专用，边缘更精细

---

## 三、AI改图：局部内容替换

### 3.1 功能描述

**输入**：图片 + 蒙版区域 + 提示词  
**输出**：局部替换后的图片

**典型场景**：
- 改变物体颜色："把红色汽车改成蓝色"
- 替换背景："把室内背景改成海边"
- 修改细节："把眼睛变大一点"

### 3.2 实现流程

```
用户操作流程：
1. 选中图片元素
2. 点击"AI改图"
3. 用户涂抹要修改的区域（生成蒙版）
4. 输入修改提示词："改成蓝色"
5. 点击确认
6. 系统发起改图请求
7. 显示加载状态
8. 改图完成，原地替换
```

### 3.3 关键技术

**蒙版绘制**：

```typescript
// 用户涂抹区域，生成蒙版
async function getMaskFromUserSelection() {
  // 1. 显示涂抹工具
  const brushTool = editor.createBrushTool({
    size: 20,          // 画笔大小
    color: '#ff0000',  // 红色高亮
    opacity: 0.5,      // 半透明
  });

  // 2. 用户涂抹
  const brushStrokes = await brushTool.waitForUserInput();

  // 3. 生成蒙版（黑白图）
  const mask = await generateMaskFromStrokes(brushStrokes);

  return {
    mask,              // Base64编码的蒙版图片
    bounds: brushTool.getBounds(),  // 涂抹区域的边界
  };
}
```

**核心API调用**：

```typescript
const result = await fetch('/api/ai/inpaint/replace', {
  method: 'POST',
  body: JSON.stringify({
    image_url: element.imageUrl,  // 原图
    mask: maskArea.mask,          // 蒙版区域
    prompt: "改成蓝色",            // 修改提示词
    strength: 0.8,                // 改动强度（0-1）
  }),
});

// 返回结果
{
  "result_url": "https://cdn.com/modified-image.jpg"
}
```

**前端处理**：

```typescript
// 原地替换图片
function replaceImageInPlace(element, newImageUrl) {
  // 保存原图位置和尺寸
  const { x, y, width, height, rotation } = element.transform;

  // 更新图片URL（保持其他属性不变）
  editor.updateElement(element, {
    imageUrl: newImageUrl,
  });

  // 淡入动画
  animateFadeIn(element, 300);
}
```

---

## 四、AI消除：智能去除物体

### 4.1 功能描述

**输入**：图片 + 要消除的区域  
**输出**：消除后的图片（智能填充）

**典型场景**：
- 移除照片中的路人
- 去除图片上的水印
- 删除多余的物体

### 4.2 实现流程

```
用户操作流程：
1. 选中图片元素
2. 点击"AI消除"
3. 用户涂抹要消除的区域
4. 点击确认
5. 系统智能填充消除区域
6. 显示消除结果
```

### 4.3 关键技术

**核心API调用**：

```typescript
const result = await fetch('/api/ai/inpaint/remove', {
  method: 'POST',
  body: JSON.stringify({
    image_url: element.imageUrl,
    mask: removeArea.mask,    // 要消除的区域
    algorithm: 'lama',        // 或 'telea'
  }),
});
```

**算法选择**：

- `lama`：LaMa算法，效果更好，适合复杂场景
- `telea`：传统算法，速度快，适合简单场景

---

## 五、AI扩图：智能扩展边界

### 5.1 功能描述

**输入**：图片 + 扩展方向 + 扩展尺寸  
**输出**：扩展后的图片

**典型场景**：
- 将16:9的图扩展成1:1
- 扩展图片边缘，增加留白
- 智能补全被裁切的内容

### 5.2 实现流程

```
用户操作流程：
1. 选中图片元素
2. 点击"AI扩图"
3. 选择扩展方向（上/下/左/右/全部）
4. 选择扩展比例（如20%）
5. 点击确认
6. 系统智能生成扩展内容
7. 显示扩展结果
```

### 5.3 关键技术

**核心API调用**：

```typescript
const result = await fetch('/api/ai/outpaint/expand', {
  method: 'POST',
  body: JSON.stringify({
    image_url: element.imageUrl,
    direction: 'all',             // 'left'|'right'|'top'|'bottom'|'all'
    expand_pixels: 200,           // 扩展像素数
    prompt: element.context,      // 上下文提示（可选）
  }),
});

// 返回结果
{
  "result_url": "https://cdn.com/expanded-image.jpg",
  "new_width": 2120,    // 扩展后的宽度
  "new_height": 1080,   // 扩展后的高度
}
```

**前端处理**：

```typescript
// 更新元素尺寸和URL
editor.updateElement(element, {
  imageUrl: result.result_url,
  width: result.new_width,
  height: result.new_height,
  // 保持中心位置不变
  transform: {
    ...element.transform,
    x: element.transform.x - (result.new_width - element.width) / 2,
    y: element.transform.y - (result.new_height - element.height) / 2,
  },
});
```

---

## 六、AI变清晰：图片超分辨率

### 6.1 功能描述

**输入**：低清图片  
**输出**：高清图片

**典型场景**：
- 老照片修复
- 模糊图片变清晰
- 小图放大不失真

### 6.2 实现流程

```
用户操作流程：
1. 选中图片元素
2. 点击"AI变清晰"
3. 选择放大倍数（2x/4x）
4. 点击确认
5. 系统进行超分辨率处理
6. 显示清晰化结果
```

### 6.3 关键技术

**核心API调用**：

```typescript
// 检查图片分辨率
const isLowResolution = element.width < 800 || element.height < 800;
if (!isLowResolution) {
  showToast('图片分辨率已经足够，无需变清晰');
  return;
}

const result = await fetch('/api/ai/enhance', {
  method: 'POST',
  body: JSON.stringify({
    image_url: element.imageUrl,
    scale_factor: 2,    // 放大倍数（2或4）
    denoise: true,      // 降噪
    sharpen: true,      // 锐化
  }),
});
```

**前端处理**：

```typescript
// 保持原尺寸更新（不改变画布上的大小）
editor.updateElement(element, {
  imageUrl: result.url,
  // width 和 height 保持不变
});
```

---

## 七、图文分层：自动识别文字和图形

### 7.1 功能描述

**输入**：设计海报图片  
**输出**：可编辑的模板（文字层 + 图形层 + 背景层）

**典型场景**：
- 将设计好的海报转成可编辑模板
- 提取图片中的文字内容
- 分离图片中的图形元素

### 7.2 实现流程

```
用户操作流程：
1. 选中海报图片
2. 点击"图文分层"
3. 系统自动识别：
   - 文字内容、位置、字体、颜色
   - 图形元素、路径、填充
   - 背景图层
4. 生成可编辑的模板
5. 用户可以修改文字、替换图形
```

### 7.3 关键技术

**核心API调用**：

```typescript
const result = await fetch('/api/ai/image-to-template', {
  method: 'POST',
  body: JSON.stringify({
    image_url: element.imageUrl,
    detect_text: true,       // 识别文字
    detect_graphics: true,   // 识别图形
  }),
});

// 返回结果
{
  "background_layer": "https://cdn.com/background.jpg",
  "text_layers": [
    {
      "content": "春季新品",
      "font_family": "PingFang SC",
      "font_size": 48,
      "color": "#ff0000",
      "position": { "x": 100, "y": 200 },
    },
    // ... 更多文字层
  ],
  "graphic_layers": [
    {
      "type": "shape",
      "path_data": "M 0,0 L 100,0 L 100,100 Z",
      "fill_color": "#0000ff",
      "position": { "x": 50, "y": 50 },
    },
    // ... 更多图形层
  ],
}
```

**前端处理**：

```typescript
// 创建模板结构
function createTemplateFromLayers(result) {
  const template = {
    background: result.background_layer,
    elements: [],
  };

  // 添加文字层
  result.text_layers.forEach((textLayer) => {
    const textElement = editor.createElement({
      type: 'text',
      content: textLayer.content,
      fontFamily: textLayer.font_family,
      fontSize: textLayer.font_size,
      color: textLayer.color,
      transform: textLayer.position,
    });
    template.elements.push(textElement);
  });

  // 添加图形层
  result.graphic_layers.forEach((graphicLayer) => {
    const shapeElement = editor.createElement({
      type: 'shape',
      pathData: graphicLayer.path_data,
      fill: graphicLayer.fill_color,
      transform: graphicLayer.position,
    });
    template.elements.push(shapeElement);
  });

  return template;
}

// 替换原元素为模板组
function replaceWithTemplate(element, template) {
  // 删除原图片元素
  editor.removeElement(element);

  // 添加模板元素
  template.elements.forEach((el) => {
    editor.addElement(el);
  });
}
```

---

## 八、转矢量：位图转矢量

### 8.1 功能描述

**输入**：位图（PNG/JPG）  
**输出**：矢量图（SVG）

**典型场景**：
- Logo转矢量，无损缩放
- 图标转SVG格式
- 简笔画转可编辑矢量

### 8.2 关键技术

**核心API调用**：

```typescript
const result = await fetch('/api/ai/image-to-svg', {
  method: 'POST',
  body: JSON.stringify({
    image_url: element.imageUrl,
    simplify: true,      // 简化路径
    color_mode: 'auto',  // 'auto'|'mono'|'color'
  }),
});

// 返回结果
{
  "svg_data": "<svg>...</svg>",
  "svg_url": "https://cdn.com/vector.svg",
}
```

**前端处理**：

```typescript
// 创建SVG元素替换原图片
const svgElement = editor.createElement({
  type: 'svg',
  svgData: result.svg_data,
  svgUrl: result.svg_url,
  transform: element.transform,  // 继承原位置
});

editor.replaceElement(element, svgElement);
```

---

## 九、AI滤镜：图片风格转换

### 9.1 功能描述

**输入**：图片 + 目标风格  
**输出**：风格化后的图片

**典型场景**：
- 照片转油画
- 照片转素描
- 照片转动漫风格

### 9.2 关键技术

**核心API调用**：

```typescript
const result = await fetch('/api/ai/style-transfer', {
  method: 'POST',
  body: JSON.stringify({
    image_url: element.imageUrl,
    style: 'oil_painting',  // 风格：'oil_painting'|'sketch'|'anime'
    strength: 0.8,          // 风格强度（0-1）
  }),
});
```

---

## 十、工具通用处理流程

### 10.1 统一的前端架构

所有AI工具都遵循相同的处理流程：

```typescript
// 统一的工具调用流程
async function executeAITool(toolType: string, params: any) {
  // 1. 参数验证
  validateParams(params);

  // 2. 显示加载状态
  showLoading();

  // 3. 调用API
  const result = await callAIToolAPI(toolType, params);

  // 4. 保存记录
  await saveToolRecord({
    tool_type: toolType,
    params,
    result,
    element_id: params.element_id,
  });

  // 5. 更新画布
  updateCanvasElement(result);

  // 6. 隐藏加载状态
  hideLoading();

  // 7. 埋点追踪
  tracker.trackToolCompleted({
    tool_name: toolType,
    elapsed_time: Date.now() - startTime,
  });
}
```

### 10.2 错误处理

```typescript
try {
  await executeAITool(toolType, params);
} catch (error) {
  if (error.code === 'INSUFFICIENT_BALANCE') {
    showToast('稿豆不足，请充值');
    openPaymentModal();
  } else if (error.code === 'IMAGE_TOO_LARGE') {
    showToast('图片太大，请先压缩');
  } else {
    showToast('处理失败，请重试');
  }

  // 错误埋点
  tracker.trackToolFailed({
    tool_name: toolType,
    error_code: error.code,
    error_message: error.message,
  });
}
```

### 10.3 缓存策略

```typescript
// 工具结果缓存
class ToolResultCache {
  private cache = new Map<string, Promise<any>>();

  async getOrFetch(key: string, fetcher: () => Promise<any>) {
    // 检查缓存
    if (this.cache.has(key)) {
      return this.cache.get(key);
    }

    // 发起请求
    const promise = fetcher();
    this.cache.set(key, promise);

    // 请求失败时删除缓存
    promise.catch(() => {
      this.cache.delete(key);
    });

    return promise;
  }

  clear() {
    this.cache.clear();
  }
}

// 使用示例
const cacheKey = `${toolType}_${imageUrl}`;
const result = await toolResultCache.getOrFetch(cacheKey, () => {
  return callAIToolAPI(toolType, params);
});
```

---

## 十一、总结与最佳实践

### 11.1 设计原则

1. **统一的用户体验**：所有工具都遵循相同的交互流程
2. **即时反馈**：加载状态、进度提示、结果预览
3. **容错机制**：网络异常、API失败、参数错误都有友好提示
4. **记录可追溯**：每次操作都保存记录，支持撤销和重做

### 11.2 性能优化

1. **懒加载**：工具面板按需加载
2. **结果缓存**：相同参数的请求复用结果
3. **并行处理**：批量工具可以并行执行
4. **智能预加载**：预测用户可能使用的工具，提前加载资源

### 11.3 下一步

- **上一篇**：[07-Agent工作流程](./07-Agent工作流程.md) - 了解Agent编排
- **下一篇**：[09-性能优化与错误处理](./09-性能优化与错误处理.md) - 学习系统优化

---

> **本章完成！** 您已经掌握了所有AI工具的前端实现原理。
